...
автоматические роли спикеров и облачный анализ в том же запросе (у обычного
OpenAI-совместимого API `prompt` означает совсем другое) — у остальных сервисов эти
контролы показываются неактивными, а анализ, заказанный при транскрибации, всегда
выполняется на этом Mac.
