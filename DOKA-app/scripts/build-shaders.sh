...
# как есть), а build-tool-плагин SPM ломал мультиарх-сборку
# (`swift build --arch arm64 --arch x86_64` падал на резолве плагин-таргета;
# воспроизводилось и на пустом пакете). Сейчас сборка только arm64, но
# рабочий скрипт на непроверенный плагин не меняем.
